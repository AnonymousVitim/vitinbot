...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Até mais @${num.split('@')[0]}\nEspero que um dia ele volteðŸ‘‹`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '😡Espera porra eu to fazendo, alem de ser de graça só reclama😡',
				success: '✓Sucesso✓',
                                levelon: ' ✔️ *Habilitar Level*',
				leveloff: ' ✔️ *Desabilitar Level*',
				levelnoton: '😡 *level não ativo anta*',
				levelnol: '*pqp KKK level* 0 ',
				error: {
					stick: 'PUTA QUE PARIU SÓ CONVERTE FIGURINHA NÃO ANIMADA',
					Iv: 'link invalido mano'
				},
				only: {
					group: '😐Este comando só pode ser usado em grupos animal do caralho',
					ownerG: '😡 Este comando só pode ser usado pelo meu dono😡',
					ownerB: '😡 Este comando só pode ser usado pelo  meu dono😡',
					admin: 'Este comando só pode ser usado por administradores do grupo membro comum!😳',
					Badmin: 'Este comando só pode ser usado quando o bot se torna administrador caralho😡',
                                        daftarB: `⚠️REGISTRE-SE⚠️\nOla corno(a)!\nVocé não está registrado no banco de dados, siga o exemplo abaixo para se registrar \n\nComando : ${prefix}registrar nome|idade\nExemplo : ${prefix}registrar Hidra|18`,
				}
			}
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '556993899391@s.whatsapp.net'
                        /******ApiKey Input******/
...
case 'timer':
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
				break
				case 'hidramenu':
					client.sendMessage(from, hidramenu(prefix, sender), text, {quoted: mek})
							break
							case 'menuadmin':
				...
				case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Pedido recebido, KKKK otário perdeu adm se fodeu :\n`
							teks += `@_.split('@')[0]`
						}
                  ...
                  case 'promover':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pronto carai adicionado como administrador :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}snowwrite texto1|texto2, exemplo ${prefix}snowwrite Hidra|BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}marvellogo texto, por exemplo ${prefix}marvellogo Hidra BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'audio':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Qual é a sigla de idioma?', text, {quoted: mek})
                                   if (!isUser) return reply(mess.only.daftarB)
				...
				case 'hilih':
					...
					if (Number(args[0]) === 1) {
						if (is) return reply('o recurso anti-link está ativo')
						.push(from)
						fs.writeFileSync('./database/json/.json', JSON.stringify())
						...
						if (!is) return reply('O modo de anti-link foi desativado')
						var ini = anti.indexOf(from)
						.splice(ini, 1)
						fs.writeFileSync('./database/json/.json', JSON.stringify())
						reply('Desativar anti-link com sucesso neste grupo ✔️')
					} else {
				...
				case 'tm':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*ã€Œ TRANSMISSAO ã€*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*ã€Œ TRANSMISSAO ã€*\n\n${body.slice(4)}`)
						}
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'TchauzinhooðŸ‘‹', text) // ur cods
					}, 0)
                     break
           ...
           break
				...
				case 'add':
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar, acho que o numero é privado affs')
					}
					break
				...
				case 'remover':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Tchauzinho otario, não leu as regras se fodeu😳 :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Tchauzinho otario , não leu as regras se fodeu😳: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'SE FODEU KKKKJ😳', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
				...
				case 'textoft':
					...
					break
                     ...
                     const bug = body.slice(5)
                      if (pesan.length > 300) return client.sendMessage(from, 'Desculpe, o texto Ã© muito longo, mÃ¡ximo de 300 letras', msgType.text, {quoted: mek})
	    ...
	    case 'kapankah':
			...
			case 'fechargp':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`âŒ Falhou, no momento da conversÃ£o ${tipe} para a figurinha`)
							})
							.on('end', function () {
								console.log('Finish')
				...
				case 'toimg':
				    client.updatePresence(from, Presence.composing)
                                    if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedSticker) return reply('âŒ Apenas Stickers âŒ')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('âŒ Falha ao converter Stickers em imagens âŒ')
                	...
                	client.updatePresence(from, Presence.composing) 
                        if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedVideo) return reply('ISSO SÓ FUNCIONA COM VIDEO ANIMAL')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('⚠️Fez alguma coisa errada mano KKK ⚠️')
						buffer = fs.readFileSync(ran)
                ...
                case 'ninjalogo':
                      if (args.length < 1) return reply('Cadê o texto krl?')
                      if (!isUser) return reply(mess.only.daftarB)
                      gh = body.slice(11)
                      gl1 = gh.split("|")[0];
                      gl2 = gh.split("|")[1];
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `*Musica encontrada!!*\nTítulo : ${anu.result.title}\nFonte : ${anu.result.source}\nTamanho : ${anu.result.size}\n\n*ESPERE ENVIANDO POR FAVOR, NÃO SPAME, SE NÃO FOR FAVOR TENTAR NOVAMENTE*`
                ...
                break
                   ...
                   break
					...
					break
                                  case 'registrar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('Você já ta registrado, tem amnésia porra?')
					if (args.length < 1) return reply(`ParÃ¢metro incorreto \nCommand : ${prefix}registrar nome|idade\nContoh : ${prefix}registar Hidra|18`)
					...
					if (Number(args[0]) === 1) {
						...
						reply('✔️ SUCESSO boas vindas ativo')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, disable)
						fs.writeFileSync('./database/json/welkom.json', JSON.stringify(welkom))
						reply('✔️ SUCESSO desativado o recurso de boas-vindas neste grupo')
					} else {
						reply('Manda de novo digitando 1 para ativar, 0 para desativar o recurso')
					}
                                        break
                                case 'fakta':
					fakta = body.slice(1)
					...
					if (args.length < 1) return reply(mess.blank)
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
					if (tels.length > 15) return reply('Muito longo mano, não faço livros')
					reply(mess.wait)
				...
				case 'firetext':
					if (args.length < 1) return reply(mess.blank)
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
					if (tels.ength > 10) return reply('O texto esta longo, max 9 caracteres')
					reply(mess.wait)
				...
				if (isUser) return reply(mess.only.daftarB)
					...
					break
                                case 'nsfwneko':
				    try{
						if (!isNsfw) return reply('😔 *NSFW NAUM ATIVO* 😔')
                                                if (!isUser) return reply(mess.only.daftarB)
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`onde estÃ¡ o texto? exemplo ${prefix}logowolf Hidra|BOT`)
					reply(mess.wait)
					...
					if (Number(args[0]) === 1) {
						...
						reply('SUCESSO✔️ ativado o recurso nsfw neste grupo')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply('SUCESSO✔️ desativado o recurso nsfw neste grupo')
					} else {
            ...
            break
					...
					case 'nsfwarmpits':
				        ...
				        uptime = process.uptime()
                                ...
                                case 'neonlogo2':
                                        ...
                                        teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde estÃ¡ o texto hum\nExemplo: ${prefix}neonlogo2 hidraBOT`)
                                        reply(mess.wait)
                                ...
                                case 'lionlogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`teksnya mana um\nContoh: ${prefix}lionlogo Hidra|BOT`)
                                        reply(mess.wait)
			       ...
			       reply(anu.result.jadwalTV)
					break
                                ...
                                case 'shadow':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde estÃ¡ o texto hum\nExemplo: ${prefix}shadow HidraBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'coffee':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde estÃ¡ o texto hum\nExemplo: ${prefix}coffee HidraBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'lovepaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde estÃ¡ o texto hum\nExemplo: ${prefix}lovepaper HidraBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'woodblock':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde estÃ¡ o texto hum\nExemplo: ${prefix}woodblock HidraBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'fml2':
                                        ...
                                        break
                                case 'tiktok':
					if (args.length < 1) return reply('Cadê o url? E pra que ta usando TikTok KKKK?')
				...
				case 'ttp':
					...
					break
                                case 'clearall':
					if (!isOwner) return reply('Só meu dono pode usar esse comando mano😡🤙')
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('Pronto caralho😡')
					break
				case 'simi':
					if (args.length < 1) return reply('Onde esta o texto animal?')
					teks = body.slice(5)
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Modo simi ativado com sucesso ✔️')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Modo simi desativado com sucesso✔️')
					} else {
						reply('1 para ativar, 0 para desativar')
					}
					break
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Manda com a tag(@) de quem deseja fazer o clone das sombras')
